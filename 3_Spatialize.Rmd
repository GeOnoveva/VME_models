...
workflow: 
outputs: cleaned up resp (only samples with metadata) and local variability of vme density
...
# Inputs
```{r}
resp <- read.csv(file.path(savepath, paste(vme, "minus", paste(remove, collapse=", "), "resp.csv", collapse=" ")))
```
...
writeOGR(resp_spat, savepath, paste(vme, "minus", paste(remove, collapse=", "), "resp_spat"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
...
writeOGR(resp_spat_vl, savepath, paste(vme, "minus", paste(remove, collapse=", "),  "resp_spat_vl"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
...
```{r}
rm(resp_spat1)
write.csv(resp, file.path(savepath, paste(vme,"minus", paste(remove, collapse=", "),"resp new.csv", collapse=" ")))
write.csv(respw, file.path(savepath, paste(vme,"minus", paste(remove, collapse=", "), "st_dev_vme_dens.csv")), row.names = FALSE)
saveRDS(resp_spat_vl, file.path(savepath, paste(vme,"minus", paste(remove, collapse=", "), "resp_Spat_vl.rds")))
saveRDS(resp_spat, file.path(savepath, paste(vme,"minus", paste(remove, collapse=", "), "resp_spat.rds")))
```
