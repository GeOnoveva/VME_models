...
# Whole mapping area
```{r}
crs(resp_spat) <- crs("+proj=utm + zone=33 + datum=WGS84 + units=m + no_defs")
ov2 = over(y=pred, x=resp_spat)  # same as extract
ov2 <- e
```


```{r}
resp.dist0 <-buffer.dist(resp_spat["vmeind_density"], 
