...
# Select model
```{r}
m <- readRDS(m3, file = file.path(outpath, paste(vme, "model.rda" )))
```
...
# save outputs
```{r}
writeGDAL(pred.pix["vme_dens"],
          fname = file.path(outpath, paste(vme,"minus", paste(remove, collapse=", "), "pred.tif")),
          options = c("COMPRESS=DEFLATE"))

writeGDAL(pred.pix["vme_dens_var"],
          fname = file.path(outpath, paste(vme,"minus", paste(remove, collapse=", "), "pred var.tif")),
          options = c("COMPRESS=DEFLATE"))
