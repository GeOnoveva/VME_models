...
vme
```
In Norwegian this is called: "Svampsamfunn".

The definition for this VME followed @buhl2019vulnerable and @burgos2020predicting. This VME is defined by occurrence of the following indicators:
```{r echo=FALSE, message=FALSE, warning=FALSE}
indicators
```

# Goal and usage
The goal of this dataset is to highlight areas which may require special management attention on the grounds of possible high density of VME indicators, i.e., the potential presence of a VME (@bacoTaylor2023). Areas classified as 3 are deemed to have high density of any of the indicator taxa listed above as well as a narrow prediction interval (high certainty). This zone warrants strict management action. Areas classified as 2 are deemed to have a high density of any of the indicator taxa as well as a wide prediction interval (lower certainty, or higher variability of the density within the area). This zone requires the collection of additional data for a more robust assessment. Areas classified as 1 are deemed to have low density of any of the indicator taxa.
...
# Coverage
The coverage of this dataset is limited by the coverage of the predictor layers used in the model (see below). While the intention was to cover the whole of the MAREANO-multibeamed area as of 2020, the northern limit had to be set at 75&deg;N (instead of 80&deg;N) on account of the layer on bottom light, which finished there.

The available number of video lines for this area is 2228, which amount to 6031 200-m video samples.
...
(2) The model was used to generate a prediction of density of all VME indicators at every cell of a grid covering the entire area (median), as well as the 67% prediction interval, which is known to be an adequate estimate of the prediction error.

(2) Both raster datasets thus generated (median predicted density and prediction error) where binned into two categories each, and then combined as follows: high density with low error, high density with high error, and low density irrespective of error.
...
## Model results
The model outputs consist of two floating point, raster layers (continuous variables), as shown below. These datasets are available on request.
