...
# model
m <- readRDS(file.path(outpath,paste(vme,"minus", paste(substr(remove, 1,7), collapse=", "), "m1.rda" )))
...
errorr <- rast(file.path(file.path(outpath, name), paste(vme,"minus", paste(remove, collapse=", "), "pred var.tif")))
...
We also calculated Variance Explained by Cross-Validation and the Concordance Correlation Coefficient. For that purpose we split the data as follows:
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
This gave a total Variance Explained (in %) of:
```{r echo=FALSE, message=FALSE, warning=FALSE}
vecv
```

In addition, the Concordance Correlation Coefficient between predictions and observations was (in %):
```{r echo=FALSE, message=FALSE, warning=FALSE}
ccc*100
```
